...
# Copied from # https://github.com/docker-library/healthcheck/blob/master/mysql/docker-healthcheck
...
if select="$(echo 'SELECT 1' | mysql "${args[@]}")" && [ "$select" = '1' ]; then
    # we have a connection, now let's check for something in the schema
    if authUsers="$(echo 'SELECT count(*) from information_schema.tables where table_schema = "${MYSQL_DATABASE}" and table_name = "authUsers"' | mysql "${args[@]}")" && [ "$authUsers" = '1' ]; then
	    exit 0
    fi
fi
